...
/*
    This queue is not currently being used.
    Rather than waiting for an event to be validated by the server,
    we optimistically apply the event locally, and then send it to the server for validation.
    If the server approves the event, then we do nothing, since the event is already applied locally.
    If the server denies the event, then we roll back the local state to a previous snapshot that the server sends us.
*/
