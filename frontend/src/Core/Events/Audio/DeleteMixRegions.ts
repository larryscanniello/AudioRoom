...
export const DeleteMixRegions: EventNamespace<typeof EventTypes.DELETE_MIX_REGIONS> = {
    sharedState: false,

    getDispatchEvent: ({ param, emit, serverMandated }) => {
        return {
            type: EventTypes.DELETE_MIX_REGIONS,
            emit,
            serverMandated,
            transactionData: {
                transactionQueries: [],
            ...
            getEventNamespace: () => { return DeleteMixRegions; }
        };
    },
    ...
    executeSocket(socketManager: any, transactionData: TransactionData, data: any): void {
        executeSocketUtil(socketManager, { transactionData, sharedSnapshot: data.sharedSnapshot, type: EventTypes.DELETE_MIX_REGIONS });
    },
};