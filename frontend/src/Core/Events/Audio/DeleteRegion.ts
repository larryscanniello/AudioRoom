...
export const DeleteRegion: EventNamespace<typeof EventTypes.DELETE_REGION> = {
    sharedState: false,

    getDispatchEvent: ({ param, emit, serverMandated }) => {
        return {
            type: EventTypes.DELETE_REGION,
            emit,
            serverMandated,
            transactionData: {
                transactionQueries: [],
            ...
            getEventNamespace: () => { return DeleteRegion; }
        };
    },
    ...
    executeSocket(socketManager: any, transactionData: TransactionData, data: any): void {
        executeSocketUtil(socketManager, { transactionData, sharedSnapshot: data.sharedSnapshot, type: EventTypes.DELETE_REGION });
    },
};