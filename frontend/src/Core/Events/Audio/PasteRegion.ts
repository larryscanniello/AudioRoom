...
export const PasteRegion: EventNamespace<typeof EventTypes.PASTE_REGION> = {
    sharedState: false,

    getDispatchEvent: ({ param, emit, serverMandated }) => {
        return {
            type: EventTypes.PASTE_REGION,
            emit,
            serverMandated,
            transactionData: {
                transactionQueries: [],
            ...
            getEventNamespace: () => { return PasteRegion; }
        };
    },
    ...
    getLocalPayload(state: State): any {
        return {
            ...
            lastMipmapRanges: state.query('timeline').lastMipmapRanges,
        };
    },
    ...
    executeSocket(socketManager: any, transactionData: TransactionData, data: any): void {
        executeSocketUtil(socketManager, { transactionData, sharedSnapshot: data.sharedSnapshot, type: EventTypes.PASTE_REGION });
    },
};