...
export const SplitRegion: EventNamespace<typeof EventTypes.SPLIT_REGION> = {
    sharedState: false,

    getDispatchEvent: ({ param, emit, serverMandated }) => {
        return {
            type: EventTypes.SPLIT_REGION,
            emit,
            serverMandated,
            transactionData: {
                transactionQueries: [],
            ...
            getEventNamespace: () => { return SplitRegion; }
        };
    },
    ...
    executeSocket(socketManager: any, transactionData: TransactionData, data: any): void {
        executeSocketUtil(socketManager, { transactionData, sharedSnapshot: data.sharedSnapshot, type: EventTypes.SPLIT_REGION });
    },
};