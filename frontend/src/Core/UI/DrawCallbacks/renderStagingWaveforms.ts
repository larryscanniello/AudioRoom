...
export function renderStagingWaveforms(ref: React.RefObject<HTMLElement|null>, data: StateContainer, mipMap: Int8Array){
    ...
    const bucketSize = iterateAmount * Math.pow(2, currRes + 1);
    ...
    while(j<timeline[0].length && timeline[0][j].start < vpEndSamples){
        const region = timeline[0][j];
        const regionStartSamples = region.start;
        ...
        // Apply live slip offset: shift mipmap read index by delta samples
        const liveDelta = (data.liveSlip?.regionId === region.id) ? data.liveSlip.delta : 0;
        const slipBuckets = Math.round(liveDelta / bucketSize);
        mipMapIndex += slipBuckets;

        // Valid mipmap range for this region (built for [region.start, region.end])
