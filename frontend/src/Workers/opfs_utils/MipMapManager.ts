        ...
        for (let i = 0; i < timelines.length; i++) {
            ...
            while (currPos < endSample) {
                ...
                if (!region) {
                    ...
                    bounces[region.bounce].takeHandles[region.name].read(subarray, { at: (currPos - region.start + region.clipOffset + region.latencyOffset) * Float32Array.BYTES_PER_ELEMENT });
                    currPos += toFill;
                    bufferPos += toFill;
                }
            }
